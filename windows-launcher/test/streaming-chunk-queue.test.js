...
const { createStreamingChunkQueue } = require("../renderer/streaming-chunk-queue");
...
function makeQueue({ isCurrent = () => true } = {}) {
  ...
  function resolveSynth(text, audioBlob) {
    pendingSynth.get(text).resolve(audioBlob);
  }

  async function play(audioBlob, text) {
    playCalls.push({ audioBlob, text });
  }
...
test("pushChunk before run() starts: the chunk is synthesized and played, then onIdle fires", async () => {
  ...
  q.resolveSynth("hello", "hello-audio");
  await runPromise;

  assert.deepEqual(q.playCalls, [{ audioBlob: "hello-audio", text: "hello" }]);
...
test("pushChunk after run() has started and is waiting for the next chunk", async () => {
  ...
  q.resolveSynth("late", "late-audio");
  await runPromise;

  assert.deepEqual(q.playCalls, [{ audioBlob: "late-audio", text: "late" }]);
  assert.equal(q.idleCalls(), 1);
});
...
test("cancelPending() mid-flight drops queued-but-not-dequeued chunks but lets the in-flight one finish", async () => {
  ...
  assert.deepEqual(q.synthesizeCalls, ["first"]);
  assert.deepEqual(q.playCalls, [{ audioBlob: "first-audio", text: "first" }]);
  assert.equal(q.idleCalls(), 1);
});
...
test("run() stops silently once isCurrent() flips false, without calling onIdle", async () => {
  ...
  current = false;
  q.resolveSynth("only", "only-audio");
  await runPromise;
