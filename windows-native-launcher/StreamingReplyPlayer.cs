...
internal sealed class StreamingReplyPlayer
{
    ...
    public async Task<(string? Reply, bool Changed, string? Expression, bool Interrupted, IReadOnlyList<string> Pending)> StreamReplyAndPlayAsync(
        string commandText, string screenText = "")
    {
        var sentences = Channel.CreateUnbounded<string>();
        ReplyStreamEvent? finalEvent = null;

        var readTask = ReadEventsAsync(commandText, screenText, sentences.Writer, e => finalEvent = e);
    ...
    private async Task ReadEventsAsync(string commandText, string screenText, ChannelWriter<string> writer, Action<ReplyStreamEvent> onFinal)
    {
        try
        {
            await foreach (var evt in backendClient.ReplyStreamAsync(commandText, screenText))
            {
