...
internal sealed class StreamingReplyPlayer
{
    ...
    public async Task<(string? Reply, bool Changed, string? Expression, bool Interrupted, IReadOnlyList<string> Pending)> StreamReplyAndPlayAsync(
        string commandText, string? sessionId = null)
    {
        var sentences = Channel.CreateUnbounded<string>();
        ReplyStreamEvent? finalEvent = null;

        var readTask = ReadEventsAsync(commandText, sessionId, sentences.Writer, e => finalEvent = e);
    ...
    private async Task ReadEventsAsync(string commandText, string? sessionId, ChannelWriter<string> writer, Action<ReplyStreamEvent> onFinal)
    {
        try
        {
