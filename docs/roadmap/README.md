...
| Zed External Agent entry point | Done | Issue #24 closed. PR #25 merged. Mana has a local ACP entry point for Zed External Agents with proposal-only edit behavior. |
...
2. Resolve PR #5 and decide the first implementation slice for speech recognition accuracy.
3. Triage issue #6 because much of the market-analysis MVP appears implemented already.
4. Choose the next new feature lane: screen perception (#1), gaming performance (#2), or native launcher parity.
