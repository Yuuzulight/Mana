...
below for 14 live examples of exactly this).
...
## Implemented per GitHub, doc not yet updated (14)
...
| issue-490-openbot-governance-scoping.md | #490 | OpenBot-style multi-agent governance -- scoping verdict is not-adopted; GitHub issue itself still open |
...
**What this did NOT cover, and is real follow-up work**: main gained many
other closed issues during those 815 commits (#439, #450, and surely
others across the repo's 251 total issues) with no corresponding
`docs/roadmap/*.md` file at all, and likely other docs among the existing
111 shared files are themselves stale the same way the 14 above were --
this refresh only checked the specific batch this branch had filed, not
every doc in the directory against fresh GitHub state. A proper full
re-audit (matching the original methodology: read every doc, cross-check
every doc's linked issue against real GitHub state, not just the doc's
own prose) is a bigger task than this pass attempted. Also noted in
passing: issues #502 and #503 are exact-title duplicates of #498, created
by some other process -- worth closing as duplicates, not otherwise acted
on here.
