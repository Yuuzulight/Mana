...
// Module-level singleton, same pattern as telegram-bridge's -- one
// bridge/client shared across every route and the Gateway connection.
let bridge = null;
let client = null;
...
function startClient(deps) {
  ...
  client.on(Events.MessageCreate, (message) => {
    handleDiscordMessage({ message, bridge: activeBridge }).catch((e) =>
...
module.exports = {
  ...
  description:
    "Message Mana remotely via Discord DMs, gated by a pairing-code approval so an unknown channel can't reach her. DM-only, text-only, added alongside the Telegram bridge as a second remote-messaging option.",
